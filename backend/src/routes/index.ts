...
import authRoutes from './auth' // Novo arquivo
...
// Rotas PÚBLICAS de autenticação (SEM isAuth)
routes.use('/auth', authRoutes) // Agora /user/login, /user/register, etc. são públicos

// Rotas PROTEGIDAS (COM isAuth)
routes.use('/user', isAuth, userRoutes) // /user/myaccount, /user/id/:id, etc. protegidos
