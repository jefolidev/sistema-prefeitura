...
const Header = () => {
  ...
  return (
    <div className="header header-sticky p-0">
      <div
        className={['container-fluid', styleSidebar.sidebarHeaderFixed].join(
          ' '
        )}
      >
        <button
        ...
        <ul className="header-nav">
          <CDropdown variant="nav-item">
            ...
            <CDropdownMenu>
              ...
              <CDropdownDivider />
              {isSuperUser && (
                <CDropdownItem onClick={() => navigate('/permissoes')}>
                  {' '}
                  <CIcon icon={cilLockLocked} /> Permissões
                </CDropdownItem>
              )}
              <CDropdownItem onClick={logout}>
                <CIcon icon={cilPowerStandby} /> Desconectar
              </CDropdownItem>
            </CDropdownMenu>
          </CDropdown>
        </ul>
